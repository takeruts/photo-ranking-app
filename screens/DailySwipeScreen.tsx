...
interface DailySwipeScreenProps {
  navigation: any;
}

export default function DailySwipeScreen({ navigation }: DailySwipeScreenProps) {
  ...
  async function loadDailyPhotos() {
    ...
    try {
      ...
      if (!data || data.length < 5) {
        showAlert('エラー', '評価できる写真が不足しています（最低5枚必要です）');
  ...
  async function handleSwipe(liked: boolean) {
    ...
    try {
      ...
      if (currentIndex < photos.length - 1) {
        ...
        // 5枚の評価が完了したらlast_daily_swipe_dateを今日の日付に更新
        await supabase
          .from('profiles')
          .update({ last_daily_swipe_date: new Date().toISOString().split('T')[0] })
          .eq('id', user.id);

        // ホーム画面へ
        showAlert('完了', '本日の評価が完了しました！ありがとうございます。');
        navigation.replace('Home');
      }
    } catch (error: any) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>本日の評価</Text>
